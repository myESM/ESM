...
current_rules_version=`curl -X POST "https://api.esm.cyber.cstilab.co/esmapi/web/file/fileVersion" -d "{'TypeCode': 'it'}" -H "Content-Type: application/json" -H "accept: */*" -H "authorization: $ESM_API" | cut -d : -f 2 | cut -d \" -f 2`
...
then
    ...
    curl -o /tmp/rules.tgz "https://api.esm.cyber.cstilab.co/esmapi/web/file/download/it/$current_rules_version" -H "accept: */*" -H "authorization: $ESM_API"
    mkdir -p /tmp/rules
