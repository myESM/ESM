...
def update():
    ...
    if lastest_ver[0] != prev_ver[0]:
        ...
        subprocess.call('docker rmi $(docker images -q) > /dev/null', shell=True)
        subprocess.call('bash Install.sh', shell=True)
        tprint(f'{bcolors.OKGREEN}更新完成')
        tprint(f'{bcolors.WARNING}本次升級需重新設定 SecBuzzerESM.env')
        sys.exit()
    if lastest_ver[1] != prev_ver[1]:
        # rsync
