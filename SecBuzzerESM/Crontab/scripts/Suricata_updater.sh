...
current_rules_version=`curl -X POST "https://test.api.secbuzzer.co/esmapi/web/file/fileVersion" -H "accept: */*" -H "authorization: $ESM_API" | cut -d : -f 2 | cut -d \" -f 2`
...
then
    ...
    curl -o /tmp/rules.tgz "https://test.api.secbuzzer.co/esmapi/web/file/download/$current_rules_version" -H "accept: */*" -H "authorization: $ESM_API"
    mkdir -p /tmp/rules
